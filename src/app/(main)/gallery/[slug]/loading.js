function SkeletonBlock({ className = "" }) {
  return (
    <div
      className={`animate-pulse rounded-xl bg-stone-800/60 ${className}`.trim()}
      aria-hidden="true"
    />
  );
}
  ...
  return (
    <main className="relative z-10 w-full">
      <article className="relative mx-auto w-full max-w-6xl px-6 pb-24 pt-28 sm:px-10 sm:pt-32 lg:px-12">
        <div
          className="relative w-full overflow-hidden rounded-sm bg-stone-900/60"
          style={{ aspectRatio: "3 / 4" }}
          aria-hidden="true"
        >
          <div className="h-full w-full animate-pulse bg-linear-to-br from-stone-800/60 via-stone-900/40 to-stone-800/60" />
        </div>
