...
/**
 * Scroll cap for {@link SelectListbox}, merged onto `ListboxOptions`.
 *
 * Headless UI’s anchor layer sets **inline** `max-height` as
 * `min(var(--anchor-max-height, 100vh), <availableHeight>)` (see `@headlessui/react` floating helper),
 * which wins over Tailwind `max-h-*`. We must set `--anchor-max-height` so flipped menus stay short
 * enough to fit on-screen; overflow scrolling is applied inline by the same layer (`overflow: auto`).
 */
export function listboxDropdownScrollArea() {
  return "min-h-0 [--anchor-max-height:min(15rem,min(45vh,calc(100dvh-10rem)))] overflow-x-hidden overscroll-y-contain py-1 [scrollbar-gutter:stable]";
}

/** Floating listbox shell (border, width, z-index). Combine with {@link listboxDropdownScrollArea}. */
/** @param {boolean} light */
