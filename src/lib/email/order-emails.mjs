...
function fulfillmentBlockText(fulfillment) {
  ...
  const orderId =
    fulfillment.providerOrderId ?? fulfillment.printfulOrderId ?? null;
  const status =
    fulfillment.providerStatus ?? fulfillment.printfulStatus ?? null;
...
function fulfillmentBlockHtml(fulfillment) {
  ...
  const orderId =
    fulfillment.providerOrderId ?? fulfillment.printfulOrderId ?? "—";
  const status =
    fulfillment.providerStatus ?? fulfillment.printfulStatus ?? "—";
  return `<p><strong>${escapeHtml(fulfillment.provider || "Fulfillment")}</strong><br/>Order id: ${escapeHtml(String(orderId))}<br/>Status: ${escapeHtml(String(status))}</p>`;
}
