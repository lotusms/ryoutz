...
export default function HomeAwardsStrip({ awards = AWARDS }) {
  ...
  return (
    <section
      ...
      className="relative z-10 overflow-hidden border-y border-amber-200/30 py-12 sm:py-14"
    >
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0"
      >
        <div className="absolute inset-0 bg-linear-to-b from-stone-50 via-amber-50/70 to-stone-100/95" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_90%_70%_at_50%_-20%,rgba(254,252,232,0.9),transparent_55%),radial-gradient(ellipse_60%_50%_at_100%_60%,rgba(251,191,36,0.18),transparent_50%),radial-gradient(ellipse_55%_45%_at_0%_70%,rgba(253,230,138,0.22),transparent_48%)]" />
        <div className="absolute -left-[18%] top-1/2 aspect-square w-[min(55vw,24rem)] -translate-y-1/2 rounded-full bg-amber-200/45 blur-[88px]" />
        <div className="absolute -right-[12%] top-[8%] aspect-square w-[min(48vw,20rem)] rounded-full bg-amber-300/30 blur-[96px]" />
        <div className="absolute bottom-0 left-1/2 aspect-2/1 w-[min(90%,42rem)] -translate-x-1/2 translate-y-1/3 rounded-full bg-stone-200/50 blur-3xl" />
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 sm:px-10 lg:px-12">
        <p className="text-center text-[11px] font-medium uppercase tracking-[0.32em] text-stone-700/90">
          WeddingWire
        </p>
        <h2
          id="home-awards-heading"
          className="mt-2 text-center font-serif text-3xl font-medium tracking-[-0.02em] text-stone-950 sm:text-4xl"
        >
          Awards & Recognition
        </h2>
